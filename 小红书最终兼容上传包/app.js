(function () {
  ...
  function playSfx(kind) {
    ...
    if (kind === "found") {
      note(1318.5, t, 0.3, "sine", 0.13);
      note(1760, t + 0.06, 0.38, "sine", 0.1);
    } else if (kind === "chime") {
      var scale = [523.25, 587.33, 659.25, 783.99, 880];
      scale.forEach(function (freq, index) { note(freq, t + index * 0.09, 0.5, "triangle", 0.16); });
    } else if (kind === "wrong") {
      note(150, t, 0.38, "sawtooth", 0.15);
      note(142, t + 0.02, 0.38, "sawtooth", 0.13);
    } else if (kind === "stamp") {
      note(120, t, 0.3, "square", 0.2);
      note(92, t + 0.05, 0.28, "sine", 0.17);
    }
  }

  function buzz(pattern) {
    try { if (navigator.vibrate) navigator.vibrate(pattern); } catch (e) { void e; }
  }

  function shake() {
    var card = root.querySelector(".play-card");
    if (!card) return;
    card.classList.remove("shake");
    void card.offsetWidth;
    card.classList.add("shake");
    visualTimers.push(window.setTimeout(function () { card.classList.remove("shake"); }, 460));
  }
  ...
  function toast(html) {
    ...
    visualTimers.push(window.setTimeout(function () { if (el.parentNode) el.parentNode.removeChild(el); }, 2100));
  }

  function hintText() {
    if (state.scene === "light") {
      if (state.found.length === 0) return "从镜面缓缓扫过：右上方与镜缘各有一处反光异常。";
      if (state.found.length < 3) return "还有痕迹藏在镜缘附近，把灯贴近边缘慢慢移动。";
      return "三处痕迹已齐，点击下方「转入听音台」。";
    }
    if (state.scene === "sound") {
      if (state.soundAttempts >= 1) return "正确顺序：玉磬 → 木鼓 → 铜钲 → 玉磬（清·暖·沉·清）。";
      return "第一声与最后一声相同；先记住音高，再按顺序复现。";
    }
    if (state.scene === "rubbing") {
      if (state.rubbed.length < 9) return "按住拓纸横扫空白区域即可，不必逐格点击。";
      return "铭文已可读，可继续拓满全部 12 区以获成就。";
    }
    if (state.scene === "deduction") return "作坊铭只证明产地；看谁的职业会让手常年触碰镜背。";
    return "跟着顶部步骤提示走，卡住随时点「提示」。";
  }

  function badges() {
    var list = [];
    if (state.soundState === "solved" && state.soundAttempts === 1) list.push({ t: "过耳不忘", d: "听音一次即复现" });
    if (state.rubbed.length >= 12) list.push({ t: "纤毫毕现", d: "拓满全部十二区" });
    return list;
  }
  ...
  function headerHtml() {
    ...
    return '<header class="topbar game-topbar">' +
      ...
      '<div class="header-actions"><button class="hint-button" data-action="hint">提示</button>' +
      '<button class="evidence-button" data-action="bag">证物袋 <b>' + evidence().length + '</b></button>' +
  ...
  function coverHtml() {
    return '<section class="hero case-cover scene-panel"><div class="case-stamp">1122<br><small>宣和四年</small></div>' +
      ...
      '<div class="case-object"><div class="halo"></div><div class="mirror static-mirror"><span class="mirror-ring ring-one"></span><span class="mirror-ring ring-two"></span><span class="mirror-heart" data-action="egg">玄</span><span class="hidden-mark">羽</span></div>' +
  ...
  function endingHtml() {
    ...
    var earned = badges();
    var badgeHtml = earned.length
      ? '<div class="badge-list">' + earned.map(function (item) {
          return '<span class="badge"><b>' + item.t + '</b><small>' + item.d + '</small></span>';
        }).join("") + '</div>'
      : "";
    return '<section class="ending scene-panel ' + (state.suspect === "musician" ? "true-ending" : "") + '"><div class="ending-card">' +
      ...
      '<div class="ending-evidence"><span>你选择了 ' + person.name + '</span><span>关键证物：' + state.reason + '</span><span>听音尝试：' + state.soundAttempts + ' 次</span></div>' +
      badgeHtml + '</div>' +
  ...
  function restart() {
    clearVisualTimers();
    var lingering = document.querySelector(".toast");
  ...
  function moveLight(event, stage) {
    ...
    if (changed) {
      playSfx("found");
      buzz(30);
      toast("新证物入袋 · <b>" + clues[newFound].title + "</b>");
  ...
  function revealTile(button) {
    ...
    if (state.rubbed.length === 9) {
      playSfx("chime");
      toast("铭文可读：<b>湖州石家青铜照子</b>");
    } else if (state.rubbed.length === 12) {
      playSfx("found");
      buzz(30);
      toast("成就解锁：<b>纤毫毕现</b> · 拓满十二区");
    }
  ...
  function bindEvents() {
    root.querySelectorAll("[data-scene]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        var next = button.getAttribute("data-scene");
        if (next === "ending") {
          playSfx("stamp");
          buzz(60);
        }
    ...
    root.querySelectorAll('[data-action="hint"]').forEach(function (button) { button.addEventListener("click", function () { toast(hintText()); }); });
    var egg = root.querySelector('[data-action="egg"]');
    if (egg) {
